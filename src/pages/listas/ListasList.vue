<template>
  <q-page-async :loading="loading" padding>
    <q-table
      ...
      no-data-label="Nenhuma lista registrada"
    >
      <template v-slot:top>
        <div class="full-width">
          <p class="text-h6 text-weight-regular q-mb-sm">Listas de compras</p>
    ...
    <q-dialog v-model="dialogEditor" @hide="resetarFormulario">
      <q-card style="width: 26rem; max-width: 100%">
        <q-form @submit.prevent="salvarItem">
          <q-toolbar>
            <q-toolbar-title>{{ iptId ? 'Editar' : 'Cadastrar' }} lista</q-toolbar-title>
...
export default defineComponent({
  ...
  setup() {
    ...
    return {tableColumns, tableRows, tablePagination, tableFilter}
  },
  data: () => ({
    loading: true,
    dialogEditor: false,
  ...
  methods: {
    ...
    async removerItem(id, nome) {
      this.$q.dialog({
        title: 'Remover lista',
        message: `A lista "${nome}" será removida.`,
        html: true,
        cancel: true,
        persistent: false,
      }).onOk(async () => {
    ...
    async salvarItem() {
      ...
      try {
        ...
        if (this.iptId) await this.$api.put('/crud', {table: 'listas', data})
        else await this.$api.post('/crud', {table: 'listas', data})
        await this.loadData(false)
